...
El conjunto de datos base para este estudio proviene de una extracción realizada mediante técnicas de _web scraping_ sobre el portal del Registro Único Empresarial y Social (RUES) de Colombia #cite(label("rues")). La importancia de este dataset radica en su capacidad para ofrecer una radiografía actual del tejido empresarial formal en diversas regiones del país, permitiendo analizar patrones de constitución, formalización y supervivencia de las organizaciones.

El archivo original cuenta con 4377 registros y 17 columnas. Las variables capturadas incluyen identificadores únicos (NIT), ubicación geográfica (*Cámara de Comercio*), información temporal (*Fecha de Matrícula*, *Renovación*, *Cancelación*) y categorización jurídica (*Tipo de Sociedad*, *Tipo de Organización*).

El problema analítico que se pretende abordar consiste en determinar si existen factores estructurales, tanto geográficos como macroeconómicos, que condicionen la elección del tipo societario y la pervivencia de las empresas. Para responder a esto, se hace necesario enriquecer la información registral, que es de carácter administrativo, con datos del contexto económico.
...
El proceso de integración se realizó mediante un script en Python que consume directamente la API del Banco Mundial. Se seleccionaron cinco indicadores clave: crecimiento del PIB anual, inflación al consumidor, tasa de desempleo total, tasa de interés para préstamos y remesas como porcentaje del PIB. Estos datos, originalmente en formato "ancho" (años como columnas), fueron transformados mediante una operación de _pivot_ para alinearlos con la estructura tabular requerida.

Se cruzó el año de la *Fecha de Matrícula* con los indicadores económicos para capturar el entorno de nacimiento de la empresa.

El resultado es un dataset enriquecido donde cada empresa no solo tiene sus atributos legales, sino también una "foto" económica de su nacimiento.

= Limpieza de datos

La fase de limpieza fue crítica debido a la naturaleza heterogénea de los datos extraídos de la web. Se identificaron inconsistencias en los formatos de fecha y cadenas de texto no estandarizadas.

En primer lugar, las columnas temporales (*Fecha de Matrícula*, *Vigencia*, etc.) se convirtieron al formato `datetime`. Se observó que una proporción significativa de registros carecía de fechas válidas o presentaba el valor "Indefinido" en la vigencia; estos casos se gestionaron extrayendo únicamente el año para los cruces, imputando con cero o descartando registros cuando la fecha de matrícula era inexistente, ya que es una variable indispensable para la integración económica.

Respecto a las variables categóricas, se detectó un desbalanceo severo en la variable objetivo *Tipo de Sociedad*. La clase "Sociedad Comercial" representa cerca del 90% de los datos, mientras que categorías como "Economía solidaria" tenían una presencia casi anecdótica. Para los análisis supervisados y las pruebas de hipótesis, se aplicaron filtros para excluir clases con menos de dos instancias, evitando errores en la estratificación durante la división de conjuntos de entrenamiento y prueba. Asimismo, para la prueba Chi-cuadrado, se agruparon las regiones minoritarias bajo la etiqueta "Otras Regiones" y los tipos societarios poco frecuentes bajo "Otros Tipos", asegurando así la robustez estadística de los resultados.
...
Se aplicó el algoritmo K-Means combinado con una reducción de dimensionalidad PCA (Análisis de Componentes Principales). El objetivo fue identificar agrupaciones naturales de empresas basadas en su ubicación y las condiciones económicas de su fundación. Se determinó un número óptimo de 4 clústeres.
...
Se entrenó un modelo _Random Forest_ para predecir el *Tipo de Sociedad*. A pesar de obtener una exactitud general del 63.82%, el análisis detallado reveló dificultades asociadas al desbalanceo de clases. El modelo logró un alto _recall_ (90%) para la clase minoritaria "Sociedad Civil", pero con una precisión muy baja, generando numerosos falsos positivos.
